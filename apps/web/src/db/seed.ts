...
async function main() {
  ...
  const kpiRows: Array<typeof schema.kpiDefinitions.$inferInsert> = [
    ...
    {
      kpiCode: "TRAINEE_INCIDENTS",
      name: "Trainee incidents",
      definition: "Incidents where person_type = trainee.",
      formula: "count(incidents where person_type='trainee')",
      unit: "count",
      classification: "lagging",
      direction: "lower_better",
      reportingFrequency: "monthly",
    ...
    {
      kpiCode: "REPORTABLE_OSH_CASES",
      name: "Reportable OSH cases",
      definition:
        "Incidents with a statutory OSH-notification status of 'yes', per the applicable OSH regime (Mauritius OSH Act in scope for v1). Deliberately distinct from hospital_referral.",
      formula: "count(incidents where reportable_status='yes')",
      unit: "count",
      classification: "lagging",
      direction: "lower_better",
      reportingFrequency: "monthly",
